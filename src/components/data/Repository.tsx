...
private getStatusBadgeUrl(workflow_filename : string) : string {
    return this.repository_url + "/actions/workflows/" + workflow_filename + "/badge.svg";
}
private getLatestRelease(): string {
    let tag: string = 'None';
    console.log({owner : this.owner, repo : this.repository_name})
    this.octokit.rest.repos.getLatestRelease({owner : this.owner, repo : this.repository_name})
    .then(response => {
        tag = response.url;
...
constructor(repository_name : string, owner : string = "OUXT-Polaris"){
    ...
    this.owner = owner;
    this.repository_url = "https://github.com/" + owner +"/" + this.repository_name
